...
{else}
	...
	<div{if $field->required || $field->errors() || $class} class="{trim}{if $field->required}required{/if}{if $field->errors()} error_field{/if}{if $class} {$class}{/if}{/trim}"{/if}>
		{if $reverse}{$field->as_widget()}{/if}
		<label for="{$field->get_id()}"{if $hide_label} class="access"{/if}>{$field->label}{if $field->required}<span> ({t}required{/t})</span>{/if}</label>
		{if !$reverse}{$field->as_widget()}{/if}
